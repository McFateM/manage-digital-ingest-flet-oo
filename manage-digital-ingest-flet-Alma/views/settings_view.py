...
class SettingsView(BaseView):
    ...
    def render(self) -> ft.Column:
        ...
        mode_settings_container = ft.Container(
            content=ft.Column([
                ft.Text("Processing Mode: Alma", size=16, weight=ft.FontWeight.BOLD, color=colors['primary_text']),
                ft.Text("This app is configured for Alma workflows only", size=12, italic=True, color=colors['secondary_text'])
            ]),
            padding=ft.padding.all(8),
            border=ft.border.all(1, colors['border']),
            border_radius=10,
