...
def file_selector_view(page):
    ...
    def do_fuzzy_search(page, colors):
        ...
        try:
            ...
            for filename in selected_files:
                ...
                selected_paths.append(match_path if match_path and ratio >= 90 else "No Match")
    ...
    if current_file_option == "Google Sheet":
    ...
    if current_file_option == "CSV":
        ...
        def on_column_selection_change(e):
            ...
            # Make Step 4 visible if we have files to process
            step4_container = page.session.get("_step4_container")
            if step4_container:
                step4_container.visible = bool(page.session.get("selected_files"))
                page.update()  # Update the UI immediately after changing visibility
        ...
        def update_csv_display():
            ...
            if current_csv_columns:
                logger.info(f"update_csv_display - columns: {current_csv_columns}")
            if current_csv_error:
                logger.warning(f"update_csv_display - error: {current_csv_error}")
            ...
            # Clear and update the main display
            ...
            if current_selected_column:
                ...
                def on_search_dir_picker_result(e: ft.FilePickerResultEvent):
                    if e.path:
                        # Store the selected search directory in session
                        page.session.set("search_directory", e.path)
                        logger.info(f"Selected search directory: {e.path}")
                        # Update display
                        update_csv_display()

                # Create directory picker
                search_dir_picker = ft.FilePicker(
                    on_result=on_search_dir_picker_result
                )
                page.overlay.append(search_dir_picker)

                def open_search_dir_picker(e):
                    search_dir_picker.get_directory_path(
                        dialog_title="Select Directory for Fuzzy Search"
                    )

                # Get current search directory from session
                search_directory = page.session.get("search_directory")
                ...
                # Create and store the results expansion tile
                results_tile = ft.ExpansionTile(
                ...
                page.session.set("_results_tile", results_tile)
                results_display_container.content = results_tile
                
                # Create Step 4 container first
                step4_container = ft.Container(
                            content=ft.Column([
                                ft.Text("Step 4: Fuzzy Search", size=16, weight=ft.FontWeight.BOLD),
                                ft.Text("Select a directory to search for matching files:", size=14),
                                ft.Row([
                                    ft.ElevatedButton(
                                        text="Select Search Directory",
                                        on_click=lambda e: search_dir_picker.get_directory_path(
                                            dialog_title="Select Directory for Fuzzy Search"
                                        )
                                    ),
                                    ft.ElevatedButton(
                                ...
                                ft.Container(
                                    content=ft.Column([
                                        ft.Text(
                                            value="Fuzzy Search Results:", 
                                            size=14, 
                                            weight=ft.FontWeight.BOLD,
                                            visible=page.session.get("selected_paths") is not None
                                        ),
                                        ft.Column(
                                            controls=[],
                                            scroll=ft.ScrollMode.AUTO,
                            ...
                            border_radius=10,
                            bgcolor=colors['container_bg'],
                            visible=bool(selected_files)  # Set visibility based on whether files are selected
                        )
                page.session.set("_step4_container", step4_container)

                # Store step4_container in results_content
                results_display_container.content = ft.Column([
                    results_content,
        ...
        csv_main_column = ft.Column([
            ...
            csv_file_display,
            ft.Container(height=10),
            # Note has been moved to Step 3 container
        ], alignment="center")
...
def derivatives_view(page):
    ...
    log_view = ft.ListView(
        controls=[
            ft.Text("Ready to create derivatives", size=12, color=colors['primary_text'])
        ],
        height=200,
        spacing=2,
        key="derivatives_log",
    ...
    def create_derivatives_for_files():
        ...
        if not current_mode:
            ...
            log_view.controls.append(ft.Text(
        ...
        if temp_files:
            msg = f"Starting derivative creation for {total_files} files in {current_mode} mode using temporary space-free copies"
            logger.info(msg)
            log_view.controls.clear()
            log_view.controls.append(ft.Text(msg, size=12, color=colors['primary_text']))
        else:
            msg = f"Starting derivative creation for {total_files} files in {current_mode} mode using original files"
            logger.info(msg)
            log_view.controls.clear()
            log_view.controls.append(ft.Text(msg, size=12, color=colors['primary_text']))
        ...
        for index, file_path in enumerate(processing_files):
            try:
                ...
                # Update log
                log_view = page.get_control("derivatives_log")
                if log_view:
                    log_view.controls.append(ft.Text(
                    ...
                    log_view.controls[-1].focus()  # Auto-scroll to latest entry
                    page.update()
                ...
                log_view.controls[-1].focus()  # Auto-scroll
                page.update()
                processed_count += 1
        ...
        if current_mode == "Alma" and success_count > 0:
            log_view.controls.append(
                ft.Text(
                    "🔄 Renaming Alma thumbnails to .jpg.clientThumb format...",
                    size=12,
                    color=colors['primary_text']
                )
            )
        ...
        final_msg = f"✅ Completed! Processed: {processed_count}, Success: {success_count}, Errors: {error_count}"
        log_view.controls.append(
            ft.Text(
                final_msg,
                size=12,
                color=colors['primary_text'] if error_count == 0 else colors['error']
            )
        )
        log_view.controls[-1].focus()  # Auto-scroll
